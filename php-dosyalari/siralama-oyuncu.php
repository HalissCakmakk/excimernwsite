...
?>
			...
			<br>
                ...
                <div class="siralama">
					...
					<table class="siralama-table" style="table-layout: fixed; width: 100%;">
							<?php
							$factions_siralama = $db->prepare("SELECT * FROM ".$factions_db." WHERE username = ?");
							$factions_siralama->execute(array($_GET["username"]));		
							...
							echo"<div style='width: 100%; padding-top: 20px; padding-bottom: 20px;' class='flag note note--error'><center>Oyuncuya ait sıralama verileri bulunamadı!</center></div>";
							}
							?>
                        </table>
                    </div>
                    <div id="tab-2" class="tab-content">
						...
						<table class="siralama-table" style="table-layout: fixed; width: 100%;">
							<?php
							$sg_siralama = $db->prepare("SELECT * FROM ".$sg_db." WHERE username = ?");
							$sg_siralama->execute(array($_GET["username"]));	
					...
					<table class="siralama-table" style="table-layout: fixed; width: 100%;">
							<?php
							$pvp_siralama = $db->prepare("SELECT * FROM ".$pvp_db." WHERE username = ?");
							$pvp_siralama->execute(array($_GET["username"]));		
